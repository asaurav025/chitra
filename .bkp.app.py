...
def get_conn():
    return db_async.connect(DB_PATH)
...
def upload_photos():
    ...
    def upload_single_file(filename: str, file_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            db_async.upsert_photo(conn, **meta)
...
def add_photo_tags(photo_id: int):
    ...
    for item in tags:
        ...
        if not tag:
            continue
        db_async.add_tag(conn, photo_id, tag, score)
...
def process_photo_embedding(photo_id: int, file_path: str, conn):
    ...
    try:
        ...
        try:
            ...
            dim = img_vec.shape[0]
            db_async.put_embedding(conn, photo_id, vec_bytes, dim)
            
            # Auto tags
            atags = auto_tags(em, tmp_path, k=6)
            for tag, score in atags:
                db_async.add_tag(conn, photo_id, tag, float(score))
            return True
        finally:
            os.unlink(tmp_path)
    except Exception as e:
        ...
        try:
            ...
            for idx_face, f in enumerate(faces):
                ...
                if row:
                    ...
                    db_async.set_face_thumb(conn, face_id, face_thumb_path)
...
def search_photos():
    ...
    rows = db_async.get_embeddings(conn)
    if not rows:
        conn.close()
...
def cluster_faces():
    ...
    conn = get_conn()
    rows = db_async.get_faces_embeddings(conn)
    ...
    for root, members in clusters.items():
        ...
        person_id = db_async.get_or_create_person(conn, person_name)
        person_idx += 1

        for m in members:
            fid = face_ids[m]
            db_async.set_face_person(conn, fid, person_id)
...
def list_faces():
    conn = get_conn()
    rows = db_async.get_face_thumbs(conn)
    conn.close()
...
def list_persons():
    conn = get_conn()
    rows = db_async.list_persons(conn)
    conn.close()
...
def create_person():
    ...
    conn = get_conn()
    person_id = db_async.create_person(conn, name)
    conn.close()
...
def update_person(person_id: int):
    ...
    conn = get_conn()
    db_async.rename_person(conn, person_id, name)
    conn.close()
    return jsonify({"id": person_id, "name": name})
...
def assign_face_person(face_id: int):
    ...
    conn = get_conn()
    db_async.set_face_person(conn, face_id, person_id)
    conn.close()
    return jsonify({"status": "ok"})
...
def get_person_faces(person_id: int):
    ...
    conn = get_conn()
    rows = db_async.get_face_thumbs_for_person(conn, person_id)
    conn.close()
...
def merge_persons_endpoint(target_person_id: int):
    ...
    conn = get_conn()
    try:
        db_async.merge_persons(conn, source_person_id, target_person_id)
        conn.close()
