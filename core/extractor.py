from __future__ import annotations
import os, time, hashlib
from pathlib import Path
from typing import Dict, Iterable, Tuple, Optional
from PIL import Image
import imagehash
import exifread

IMG_EXTS = {'.jpg', '.jpeg', '.png', '.bmp', '.webp', '.tif', '.tiff', '.gif'}

def iter_images(root: str) -> Iterable[Path]:
    p = Path(root).expanduser()
    for dirpath, _, filenames in os.walk(p):
        for fn in filenames:
            if Path(fn).suffix.lower() in IMG_EXTS:
                yield Path(dirpath) / fn

def sha1sum(path: Path) -> str:
    h = hashlib.sha1()
    with open(path, 'rb') as f:
        for chunk in iter(lambda: f.read(1024 * 1024), b''):
            h.update(chunk)
    return h.hexdigest()

def _convert_to_degrees(value):
    """Convert GPS coordinate to decimal degrees."""
    try:
        d = float(value.values[0].num) / float(value.values[0].den)
        m = float(value.values[1].num) / float(value.values[1].den)
        s = float(value.values[2].num) / float(value.values[2].den)
        return d + (m / 60.0) + (s / 3600.0)
    except (AttributeError, ZeroDivisionError, IndexError):
        return None

def get_exif(path: Path) -> Dict:
    try:
        with open(path, 'rb') as f:
            tags = exifread.process_file(f, details=False)
        dt = str(tags.get('EXIF DateTimeOriginal') or tags.get('Image DateTime') or '')
        
        # Parse GPS coordinates
        lat = lon = None
        if 'GPS GPSLatitude' in tags and 'GPS GPSLatitudeRef' in tags:
            lat = _convert_to_degrees(tags['GPS GPSLatitude'])
            if lat and tags['GPS GPSLatitudeRef'].values[0] == 'S':
                lat = -lat
        
        if 'GPS GPSLongitude' in tags and 'GPS GPSLongitudeRef' in tags:
            lon = _convert_to_degrees(tags['GPS GPSLongitude'])
            if lon and tags['GPS GPSLongitudeRef'].values[0] == 'W':
                lon = -lon
        
        return {'exif_datetime': dt, 'latitude': lat, 'longitude': lon}
    except Exception:
        return {'exif_datetime': '', 'latitude': None, 'longitude': None}

def compute_phash(path: Path) -> str:
    try:
        img = Image.open(path).convert('RGB')
        return str(imagehash.phash(img))
    except Exception:
        return ''

def collect_metadata(path: Path) -> Dict:
    st = path.stat()
    checksum = sha1sum(path)
    ph = compute_phash(path)
    exif = get_exif(path)
    return {
        'file_path': str(path),
        'size': st.st_size,
        'created_at': time.strftime('%Y-%m-%dT%H:%M:%S', time.localtime(st.st_mtime)),
        'checksum': checksum,
        'phash': ph,
        **exif,
    }
